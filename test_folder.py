...
lr = 1e-3
batch_size = 4
test_dir = '/home/h/Desktop/data/random/test/m_label' ## 이거 transform 해서 테스트해보깅
ckpt_dir = '/home/h/unet_pytorch_testing/autocontrast/ckpt'
result_dir ='/home/h/unet_pytorch_testing/autocontrast/result'
test_lst=os.listdir(test_dir)
...
with torch.no_grad(): # no backward pass 
    ...
    for batch, data in enumerate(test_loader, 1):
        ...
        for i in range(input.shape[0]):
            ...
            bg.paste(inputimg,outputimg) ## making label
            name=data[1][i].split('/')[-1].replace('m_label', 't_output').replace('jpg','png')
            
            # outputimg.save(os.path.join(os.path.join(result_dir,'mask'), name)) ## mask
